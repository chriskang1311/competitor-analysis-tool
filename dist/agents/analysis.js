...
export async function runCompetitorAnalysis(productName, category, competitor, onProgress, segment) {
    return withRetry(() => _runAnalysis(productName, category, competitor, onProgress, segment), 3, competitor.name);
}
async function _runAnalysis(productName, category, competitor, onProgress, segment) {
            ...
            for (const block of event.message.content ?? []) {
                if (block.type === "tool_use" && onProgress) {
                    ...
                    if (block.name === "WebSearch") {
                        ...
                        try {
                            onProgress(`[${competitor.name}] 🌐 ${new URL(input.url ?? "").hostname}`);
                        }
                        catch {
                            onProgress(`[${competitor.name}] 🌐 ${input.url ?? ""}`);
                        }
                    }
                }
            }
    ...
    const searchCoverage = coverage >= 0.7 ? "full" : coverage >= 0.3 ? "partial" : "minimal";
...
export async function runAllAnalyses(productName, category, competitors, onProgress, segment) {
    const results = await Promise.allSettled(competitors.map(c => runCompetitorAnalysis(productName, category, c, onProgress, segment)));
    const analyses = [];
