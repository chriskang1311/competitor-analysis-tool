...
// ── Core search ───────────────────────────────────────────────────
export async function tavilySearch(query, maxResults = 6, days) {
...
export function formatTavilyResults(results) {
    if (results.length === 0)
        return "";
    return results
...
export async function tavilyMultiSearch(queries, maxResultsEach = 5) {
    const allResults = await Promise.all(queries.map(q => tavilySearch(q, maxResultsEach)));
 ...
 */
export async function tavilyTieredSearch(recentQueries, evergreenQueries, maxResultsEach = 5) {
