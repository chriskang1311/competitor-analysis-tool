...
function formatCompetitorList(competitors) {
...
function formatReport(report, validatedCompetitors, analyses, segment, analysisQuality) {
    const lines = [];
    ...
    validatedCompetitors.forEach((c, i) => {
        lines.push(`| ${i + 1} | ${escapeCell(c.name)} | ${escapeCell(c.company)}${segmentTag} | ${escapeCell(c.description)} | ${escapeCell(c.targetUser)} | AI-Assisted Research |`);
    });
    lines.push("");
    ...
    report.tableStakes.forEach((ts, i) => {
        lines.push(`| ${i + 1} | ${escapeCell(ts.feature)} | ${escapeCell(ts.whyExpected)} | ${escapeCell(ts.supportedBy.join(", "))} |`);
    });
    lines.push("");
    ...
    report.differentiationOpportunities.forEach((opp, i) => {
        lines.push(`| ${i + 1} | ${escapeCell(opp.opportunity)} | ${escapeCell(opp.gapDescription)} | ${escapeCell(opp.advantage)} |`);
    });
    lines.push("");
...
// ── Phase 1: Discover ────────────────────────────────────────────
async function phaseDiscover(productName, productDescription, category, reportDir, session, segment) {
    log("\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    log("🔎 PHASE 1a — Discovery Agent");
    log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");
    const rawCompetitors = await runDiscovery(productName, productDescription, category, text => log(`  ${text}`), segment);
    ...
    log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");
    const validated = await runValidator(productName, category, rawCompetitors, text => log(`  ${text}`), segment);
...
// ── Phase 2: Analyze ─────────────────────────────────────────────
async function phaseAnalyze(productName, productDescription, category, reportDir, session, selectArg, segment) {
    const validated = readJson(join(reportDir, "competitors.json"));
    // Parse selection
    ...
    if (selectArg.toLowerCase() === "all") {
        ...
        selectedCompetitors = indices.map(i => {
            if (i < 0 || i >= validated.length) {
                console.error(`❌ Invalid competitor number: ${i + 1}`);
                process.exit(1);
            }
            return validated[i];
        });
    ...
    const analyses = await runAllAnalyses(productName, category, selectedCompetitors, text => log(`  ${text}`), segment);
    ...
    log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");
    const report = await runSynthesis(productName, productDescription, category, analyses, selectedCompetitors, text => log(`  ${text}`), segment);
    ...
    const analysisQuality = new Map(analyses
    ...
    session.selectedCompetitorIndices = selectedCompetitors.map((c) => validated.findIndex(v => v.id === c.id));
    writeJson(join(reportDir, "session.json"), session);
    log(`\n✅ Report complete!\n`);
    log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
...
async function main() {
    ...
    if (sessionArg && existsSync(sessionArg)) {
        ...
        const existing = readJson(sessionArg);
        ...
        log(`\n📂 New session: ${slug}`);
        if (segment)
            log(`🎯 Segment filter: ${segment}`);
    }
    const session = readJson(join(reportDir, "session.json"));
    if (phase === "discover") {
