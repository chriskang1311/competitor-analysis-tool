...
import { buildDeepAnalysisPrompt } from "./prompts.js";
...
export async function* runDeepAnalysis(
  ...
  const prompt = buildDeepAnalysisPrompt(productName, productDescription, category, competitors);
    ...
    if (message.type === "assistant") {
      ...
      if (message.subtype === "success") {
        ...
        try {
          ...
          yield {
            type: "error",
            text: `Failed to parse analysis data: ${e instanceof Error ? e.message : String(e)}`,
          };
        }
      } else {
        const errors =
